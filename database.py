...
def is_premium(uid: int) -> bool:
    ...
    if not r["is_premium"]: return False
    if r["premium_until"]:
...
def set_premium(uid: int, days: int):
    ...
    until = (until + timedelta(days=days)).isoformat()
...
def use_referral(code: str, new_uid: int) -> int:
    """Returns referrer uid or 0"""
    db = con()
    r = db.execute("SELECT user_id, referred_by FROM users WHERE referral_code=?", (code,)).fetchone()
    ...
    if me and me["referred_by"]:
        db.close(); return 0
    db.execute("UPDATE users SET referred_by=? WHERE user_id=?", (r["user_id"], new_uid))
    db.commit(); db.close()
    return r["user_id"]
...
def get_txns(uid: int, limit: int = 0) -> list:
    db = con()
    if limit:
        rows = db.execute("SELECT * FROM transactions WHERE user_id=? ORDER BY id DESC LIMIT ?", (uid, limit)).fetchall()
    else:
        rows = db.execute("SELECT * FROM transactions WHERE user_id=? ORDER BY id DESC", (uid,)).fetchall()
    db.close()
...
def has_txn_today(uid: int) -> bool:
    ...
    db = con()
    r = db.execute("SELECT id FROM transactions WHERE user_id=? AND date=? LIMIT 1", (uid, today)).fetchone()
    db.close()
...
def add_to_goal(gid: int, uid: int, amount: float):
    db = con()
    db.execute("UPDATE goals SET current=MIN(target,current+?) WHERE id=? AND user_id=?", (amount, gid, uid))
    db.commit()
...
# ── CARDS ──────────────────────────────────────────────────────────────────────
def add_card(uid: int, last4: str, label: str = "") -> int:
    db = con()
    cur = db.execute(
        "INSERT INTO cards(user_id,last4,label,created_at) VALUES(?,?,?,?)",
        (uid, last4, label, datetime.now().isoformat()))
